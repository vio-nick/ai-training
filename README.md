...
模型接收六项 GSTRIDE 步态参数后，返回 `predicted_probability`（0--1）及等价的 `predicted_probability_percent`（0%--100%）。系统按**未经四舍五入的原始概率**固定展示为三级：低风险为 `< 30%`，中风险为 `30%--70%`（含 30% 和 70%），高风险为 `> 70%`；同时返回 `risk_level`（`low`、`medium`、`high`）和 `risk_level_display`（低风险、中风险、高风险）。
